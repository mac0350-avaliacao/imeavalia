...
fun initOferecimentos() {
    ...
    val oferecimentos: List<OferecimentoDetalhes> = gson.fromJson(content, listType)
    oferecimentos.forEach {
        var discId: Int = 0;
        var profId: Int = 0;
        transaction {
            val query = Disciplinas.select { Disciplinas.sigla eq
                    it.disciplina_sigla }.singleOrNull()
        ...
        transaction {
            val query = Professores.select { Professores.nome eq
                    it.professor_nome }.singleOrNull()
